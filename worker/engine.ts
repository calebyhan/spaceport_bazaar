    ...
    for (const entry of this.options.previous ?? []) {
      ...
      if (entry.kind === 'decision') {
        ...
        if (decision.run === s.run_id) {
          ...
          this.memory = { attempted: Object.fromEntries(Object.entries(m.attempted).map(([k, v]) => [k, BigInt(v)])), failed: m.failed ?? [], lag: BigInt(m.lag ?? 0) };
          this.state.failed = new Set(m.failed ?? []);
          this.state.lags = m.lag === undefined ? [] : [BigInt(m.lag)];
        }
      }
    }
